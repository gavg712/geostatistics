#CREADO POR PABLO CABRERA BARAHONA
#MODIFICADOR POR GABRIEL GAONA
...
#Otra forma de graficar el digrama de voronoi
library(ggplot2)
library(plotly)
library(reshape)
coord<-data.frame(x,y, Indigena) # creamos el data frame que será el espacio de trabajo
...
out.plot <- ggplot(data=coord, aes(x=x,y=y)) +
    ...
    size=2, color="#313038")
py <- plotly("gavg712", "bce1i6fb1b")
py$ggplotly(out.plot)
out.plot
